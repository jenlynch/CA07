		...
		for (String stock : sellOrders.keySet()) {
			if (buyOrders.containsKey(stock)) {
				...
				for (double price : sorted.keySet()) { //add up sell orders 
					...
					for (Order order : match) {
						if (order instanceof SellOrder) {
							totalSells += order.getSize();
						}
					}
				...
				for (double price : sorted.keySet()) { 
					...
					for (Order order : match) {
						if (order instanceof BuyOrder && price >= matchingPrice) {
							buyOrders.get(stock).remove(order);
							
							try { // delegate to trader
								order.getTrader().tradePerformed(order, matchingPrice);
							} 
							catch (StockMarketExpection e) {
								e.printStackTrace();
							}
						} else if (order instanceof SellOrder && price <= matchingPrice) {
